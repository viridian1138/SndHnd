...
/**
 * Defines the palette button for the properties editing mode.
 * 
 * @author tgreen
 *
 */
public class PaletteProperties {
	...
	public static AazonImmutableSharedEnt getRectGrpOn(Object univ) {
		if (rectGpOn.get(univ) == null) {
			...
			final AazonEnt rectL = AazonSmartOutlineRectangle.construct(
					new AazonBaseImmutableVect(-0.075, -0.075),
					new AazonBaseImmutableVect(0.055, -0.0125), AczonColor
							.getLineMagenta(), false);

			final AazonEnt rectS = AazonSmartOutlineRectangle.construct(
					new AazonBaseImmutableVect(-0.075, -0.0125),
					new AazonBaseImmutableVect(0.0, 0.01), AczonColor
							.getLineMagenta(), false);

			final AazonEnt[] entsRect = { rect , rectO, rectL , rectS };
