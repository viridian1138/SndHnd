...
public class PseudoLoopBuilder extends Panel implements MouseListener,
													 MouseMotionListener ,
	...
	public PseudoLoopBuilder(UndoManager _undoMgr) {
		super();
		undoMgr = _undoMgr;
		...
		for (countOct = 0; countOct < 10; countOct++) {
			for (countNote = 0; countNote < szz; countNote++) {
				...
				if ((nFreq > startFrequency) && (nFreq < endFrequency)) {
					...
					octMap.put(key, new Integer(countOct));
					noteMap.put(key, new Integer(countNote));
				}
			}
		}

		for ( Entry<Double,Integer> e : octMap.entrySet() ) {
			octVect.add( e.getValue() );
			noteVect.add( noteMap.get( e.getKey() ) );
		}

		int oSz = octVect.size();
		int cSz = Math.min(octVect.size(), 10);
		if (cSz == 0) {
			cSz = 1;
		}
		int cDelta = oSz / cSz;
...
protected void calcNoteDescPointsInterp(NoteDesc desc, Vector<AazonBaseMutableVect> out) {
	...
	public void editVibrato()
	{
		// InstrumentTrack track = SongData.getCurrentTrack();
		// VibratoNoteEditor vib = new VibratoNoteEditor( lastDragDesc.getDesc() , track , this );
	...
	public void editPortamento()
	{
		// InstrumentTrack track = SongData.getCurrentTrack();
		// PortamentoDesc desc = lastDragDesc.getDesc().getPortamentoDesc();
		// if( desc != null )
		// {
		//	PortamentoEditor vib = new PortamentoEditor( desc , track , this );
			...
			for (countOct = 0; countOct < 10; countOct++) {
				for (countNote = 0; countNote < szz; countNote++) {
					double nFreq = NoteTable
							.getNoteFrequencyDefaultScale_Key(countOct,
									countNote);
	...
	public void produceHarmony(final int trackNumber) throws Throwable {
		...
		if (lastDragDesc != null) {
			...
			freqRect1.setBaseFreq(1.25 * freqRect.getBaseFreq());
	...
	protected void dumpWaveform()  throws Throwable{
		final int core = 0;
		InstrumentTrack track = SongData.getCurrentTrack();
		...
		if (lastDragDesc != null) {
			...
			DraggableTransformNodeTest.start(univ1,univ2,s,null,undoMgr,new PaletteClassesDump());
