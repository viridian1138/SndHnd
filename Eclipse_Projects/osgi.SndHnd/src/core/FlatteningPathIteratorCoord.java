...
/**
 * The <code>FlatteningPathIteratorCoord</code> class returns a flattened view of
 ...
 * @author Jim Graham
 */
public abstract class FlatteningPathIteratorCoord implements PathIterator {
    ...
    /**
     * Constructs a new <code>FlatteningPathIteratorCoord</code> object that 
     ...
     */
    public FlatteningPathIteratorCoord(PathIterator src) {
	this(src, 10);
    }

    /**
     * Constructs a new <code>FlatteningPathIteratorCoord</code> object 
     ...
     *		is less than zero
     */
    public FlatteningPathIteratorCoord(PathIterator src, int limit) {
	if (limit < 0) {
    ...
    public abstract double getSquareFlat( double[] hold, int holdIndex );

    /**
     * Returns the recursion limit of this iterator.
     * @return the recursion limit of this 
     * <code>FlatteningPathIteratorCoord</code>.
     */
	...
	case SEG_QUADTO:
	    ...
	    while (level < limit) {
		if (QuadCurve2D.getFlatnessSq(hold, holdIndex) <  getSquareFlat(hold, holdIndex) ) {
		    break;
		}
	...
	case SEG_CUBICTO:
	    ...
	    level = levels[levelIndex];
	    while (level < limit) {
		if (CubicCurve2D.getFlatnessSq(hold, holdIndex) < getSquareFlat(hold, holdIndex) ) {
		    break;
		}
