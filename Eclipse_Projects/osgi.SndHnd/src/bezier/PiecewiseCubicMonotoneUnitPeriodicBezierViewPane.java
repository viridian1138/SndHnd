...
import labdaw.undo.pdx_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel_pdx_ObjectRef;
...
import abzon.AbzonCubicMonotonePeriodicCurveGenerator;
import abzon.AbzonImmutableGeneralPathFactory;
import abzon.AbzonImmutableShape;
import abzon.AbzonMonotonePeriodicCubicSlopeGenerator;
...
public class PiecewiseCubicMonotoneUnitPeriodicBezierViewPane extends JComponent implements MouseListener,
		MouseMotionListener , PropertyChangeListener {
	
	protected PiecewiseCubicMonotoneUnitPeriodicBezier piecewiseCubicMonotoneUnitPeriodicBezier = null;
	...
	protected pdx_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel_pdx_ObjectRef model = null;
	...
	public PiecewiseCubicMonotoneUnitPeriodicBezierViewPane( UndoManager _undoMgr ,
			HashMap _context , InstrumentTrack _track ,
			PiecewiseCubicMonotoneUnitPeriodicBezier _piecewiseCubicMonotoneUnitPeriodicBezier ) {
		super();
		undoMgr = _undoMgr;
		context = _context;
		track = _track;
		...
		labdaw.undo.pdx_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel_pdx_PairRef pair = labdaw.undo.pdx_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel_pdx_ObjectRef.pdxm_new_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel(
				mil , 0.0 , 0.0 , 0.0 , 0.0 );
		mil = pair.getMilieu();
		model = (labdaw.undo.pdx_PiecewiseCubicMonotoneUnitPeriodicBezierViewPaneModel_pdx_ObjectRef)( pair.getObject() );
	...
	*/
	...
	protected AczonRootFactory createSceneGraph() {
		...
		{
					double dmult = calcDomainToXYMultiplier();
					...
					Vector<AazonBaseMutableVect> interp = null;
						...
						if( ( editMode == EDIT__INTERP_POSN ) || ( editMode == EDIT__INTERP_LEVEL ) )
						{
							...
							AbzonSlopeGenerator generator = new AbzonMonotonePeriodicCubicSlopeGenerator( interp , dmult );
							
							AbzonCubicMonotonePeriodicCurveGenerator crv = new AbzonCubicMonotonePeriodicCurveGenerator( interp, generator, dmult);
			...
			Appearance lineApp, Appearance ctrlApp, boolean asgnCntl , ArrayList<AazonEnt> controlPoints)
	{
		...
		final ArrayList<InterpolationPoint> intp = piecewiseCubicMonotoneUnitPeriodicBezier.getInterpolationPoints();
...
protected void calcPointsInterp(Vector<AazonBaseMutableVect> out) {
	    ...
	    int core = 0;
		int cnta;
		final ArrayList<InterpolationPoint> intPoints = piecewiseCubicMonotoneUnitPeriodicBezier.getInterpolationPoints();
	...
	protected void calcPoints(GeneralPath out) {
		...
		int core = 0;
		int cnta;
		PiecewiseCubicMonotoneUnitPeriodicBezier bez = piecewiseCubicMonotoneUnitPeriodicBezier;
	...
	protected void updateInterpLevel(double levelU,
		...
		pt.setValue(level);
		piecewiseCubicMonotoneUnitPeriodicBezier.updateAll();
		// update efficiency
														// !!!!!!!!!!!!!!!!!!!!!!!!!!!
	}
	
	protected double calcDomainToXYMultiplier()
	{
		double d1 = 1.0 / (endParamNumber - startParamNumber);
	...
	protected void updateInterpPosn(double paramU, int index) throws Throwable {
		final int core = 0;
		ArrayList<InterpolationPoint> points = piecewiseCubicMonotoneUnitPeriodicBezier
	...
	public void refreshDisplayList() {
		...
		if( ( editMode == EDIT__INTERP_LEVEL ) || ( editMode == EDIT__INTERP_POSN ) )
		{
			ArrayList<AazonEnt> controlPoints = new ArrayList<AazonEnt>();
	...
	public void unZoom() {
		...
		setStartLevel(-40.0);
		setEndLevel(40.0);
		setStartParamNumber( -0.2 );
		setEndParamNumber( 1.2 );
	...
	{
		BezierWaveGenerateEditor editor = new BezierWaveGenerateEditor( this , 
				piecewiseCubicMonotoneUnitPeriodicBezier , track , context );
		ProgramDirector.showPropertyEditor(editor, null,
			"Bezier Wave Generate" );
	}
	...
	protected void mouserDown(MouseEvent e) {
		...
		case EDIT__INTERP_POSN: {
			...
			if( descn != null ) {
				...
				int id = descn.getId();
				int sz = piecewiseCubicMonotoneUnitPeriodicBezier.getInterpolationPoints().size();
				if ((id > 0) && (id < (sz - 1))) {
					lastDragDesc = descn;
					mouseDragEnabled = true;
				} 
