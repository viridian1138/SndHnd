...
import noise.GVfbmWaveform;
import noise.GVnoiseWaveform;
...
/**
 * 
 * An alternate snare drum agent created for the song "Sun Leaves Sky".
 * 
 * Note: like (some) real percussive instruments, SnarePercussionAgentSky plays on a particular key.  That is to say, the key of the percussion could match/mismatch the key of the rest of the song.
 ...
 *
 */
public class SnarePercussionAgentSky extends IntelligentAgent implements Externalizable {
	 ...
	 */
	public SnarePercussionAgentSky() {
		super();
		...
		GGroupWave noiseGrp = new GGroupWave();
		{
			...
			GVnoiseWaveform gv = new GVnoiseWaveform();
			gv.load( white, 0.25 );
			GVfbmWaveform vbfm = new GVfbmWaveform();
			vbfm.load( gv, 4 , 0.5 , 3 );
			noiseGrp.getEdit().getWaveOut().performAssign( vbfm );
