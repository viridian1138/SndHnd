...
/**
 * 
 * Agent emulating the playing of string 1 (the E string) on a Bronco bass guitar. 
 * 
 ...
 * 
 * Generally uses the same expr Xtal timbre generator as ExprAgentS1.
 * 
 ...
 * 
 * Timbre is intended to emulate the output of the stock Bronco bass amplifier with default knob settings.  Solutions were intentionally chosen that eliminate the background noise of the original amplifier.
 * 
 * Note: The similarity in timbre to ExprAgentS6 is really strange and extraordinary.
 * 
 * @author thorngreen
 *
 */
public class BassLoExprAgentS1 extends IntelligentAgent implements Externalizable {
	 ...
	 */
	protected double minDecayTimeBeats = 6.0;
	 ...
	 */
	protected double cutoffTimeWaves = 0.13;

	/**
	 * The duration of the initial attack of a note in waves.
	 */
	protected double attackTimeWaves = 8.75 /* 5.0 */;
	...
	/**
	 * Coefficient for the depth of the initial "pluck".
	 */
	protected double initialEffectDepthCoeff = 1.0;
	...
	/**
	 * Sets the coefficient for the depth of the initial "pluck".
	 * @param in Coefficient for the depth of the initial "pluck".
	 */
	public void setInitialEffectDepthCoeff( double in )
	{
		initialEffectDepthCoeff = in;
	}
	...
	protected void processFirstNote(NoteDesc note1, NoteDesc note2) throws Throwable {
		...
		if( ratio < 0.9999999999 )
		{
			ClampedCoefficient cb = new ClampedCoefficientRemodulator(ci, ratio);
			note1.setActualNoteEnvelope(cb);
			double endWaveNumber = note1.getFreqAndBend().getEndWaveNumber(note1, core);
	...
	public WaveForm genWaveQB()
	{
		...
		PhaseDistortionPacket pdcxf2 = new PhaseDistortionPacket(
				new SineWaveform(), 1.0,  0.11450375939849625 );
		PhaseDistortionPacket pdcxg2 = new PhaseDistortionPacket(
				new SineWaveform(), 1.0,  0.04761152882205513 );
		...
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 200.0 * ( 18771.749393228107 / 0.003536592309073662 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		coefficients.add( invg2 );
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 200.0 * ( 8446.001035972222 / 0.003536592309073662 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		
		
		WaveForm wave3 = MultiXtalBuilder.builtMultiXtalS6B();
		...
		System.out.println( "Got New Built16B!!!" );
	...
	{
		double m2 = 1.0;
		double maxDivisor = 3.22;
		ArrayList<NonClampedCoefficient> coefficients = new ArrayList<NonClampedCoefficient>();
		ArrayList<NonClampedCoefficient> coefficientCoefficients = new ArrayList<NonClampedCoefficient>();
		ArrayList<NonClampedCoefficient> parameterCoefficients = new ArrayList<NonClampedCoefficient>();
		PhaseDistortionPacket pdcxf2 = new PhaseDistortionPacket(
				new SineWaveform(), 1.0,  0.04407017543859649 );
		PhaseDistortionPacket pdcxg2 = new PhaseDistortionPacket(
				new SineWaveform(), 1.0,  0.06059649122807017 );
		PhaseDistortionPacket[] pdccxf2 = { pdcxf2 };
		PhaseDistortionPacket[] pdccxg2 = { pdcxg2 };
		WaveForm invf2 = new ArithGntWaveform( new Inverter(pdccxf2) , 4 );
		WaveForm invg2 = new ArithGntWaveform( new Inverter(pdccxg2) , 4 );
		coefficients.add( invf2 );
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 400.0 * ( 4587.599509562816 / -0.01008288009698025 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		coefficients.add( invg2 );
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 400.0 * ( 8081.00034029292 / -0.01008288009698025 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		
		
		
		WaveForm wave3 = MultiXtalBuilder.builtMultiXtalS6A();
		wave3 = new AdditiveWaveForm( wave3 , new ConstantNonClampedCoefficient( 1.0 / maxDivisor ) ,
				coefficients , coefficientCoefficients , parameterCoefficients );
		System.out.println( "Got New Built7A!!!" );
...
//		note.setRoughDraftWaveform(new SineWaveform());
	 ...
	 * @param initialEffectDepthCoeff Coefficient for the depth of the pluck.
	 * @return The phase-distorted version of the timbre
	 */
	public static GWaveForm buildBassWaveFormB(GWaveForm wave0, double initialEffectDepthCoeff ) {
		PiecewiseCubicMonotoneBezierFlat bezAC = new PiecewiseCubicMonotoneBezierFlat();
		final double strt = 1*0.15;
		bezAC.getInterpolationPoints().add(new InterpolationPoint(0.0, ( -1*0.15+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(2.0, ( -0.75*0.15+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(4.0, ( -0.25*0.15+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(6.0, ( -0.125*0.15+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(8.0, ( -0.0625*0.15+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(10.0, ( 0.0+strt ) * initialEffectDepthCoeff ));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(12.0, ( 0.0+strt ) * initialEffectDepthCoeff ));
		bezAC.updateAll();
...
/*
	public static GWaveForm buildBassWaveFormB(GWaveForm wave0) {
		PiecewiseCubicMonotoneBezierFlat bezAC = new PiecewiseCubicMonotoneBezierFlat();
		final double mult = 0.45;
		final double strt = 8 * mult;
		bezAC.getInterpolationPoints().add(new InterpolationPoint(0.0, -8*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(1.0, -4*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(2.0, -2*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(3.0, -1*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(4.0, -0.5*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(5.0, 0.0*mult+strt));
		bezAC.getInterpolationPoints().add(new InterpolationPoint(6.0, 0.0*mult+strt));
		bezAC.updateAll();
	...
	*/
	...
	public BassLoExprAgentS1() {
		...
		ina = buildBassWaveFormB( ina , initialEffectDepthCoeff );
		inb = buildBassWaveFormB( inb , initialEffectDepthCoeff );
	...
	public void initializeInitializers()
	{
		...
		NoteInitializer fb = new NoteInitializer( NoteTable.getCloseNoteDefaultScale_Key(1,
				NoteTable.STEPS_F) );
		System.out.println( fa.getBaseFreq() );
