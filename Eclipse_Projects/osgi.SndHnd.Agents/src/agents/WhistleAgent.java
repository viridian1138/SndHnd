...
/**
 * 
 * An agent generating a low-end whistle timbre similar to playing a single note on the low-pitch end of a harmonica.
 * 
 * Timbre results from the addition of sine waves.
 * 
 * @author tgreen
 *
 */
public class WhistleAgent extends IntelligentAgent implements Externalizable {
	...
	protected void buildNoteInstrument(NoteDesc note) throws Throwable {
		...
		FrequencyModulationWaveForm frm = new FrequencyModulationWaveForm( new SineWaveform() ,
				new ConstantNonClampedCoefficient( 0.65 / ( note.getFreqAndBend().getBaseFreq() ) ) );
		
		AmplitudeModulationWaveForm am = new AmplitudeModulationWaveForm( wi , frm );
		
		WaveForm wii = new TimeDiscretizedWaveForm( am , 4000.0 / 440.0 );
	...
	public WhistleAgent() {
		...
		coeffCoeff.add( new ConstantNonClampedCoefficient( 1.0 / 3.0 ) );
		coeffCoeff.add( new ConstantNonClampedCoefficient( 1.0 / 3.0 ) );
		...
		AdditiveWaveForm wi = new AdditiveWaveForm( sw , 
				new ConstantNonClampedCoefficient( 1.0 / 3.0 ) , 
