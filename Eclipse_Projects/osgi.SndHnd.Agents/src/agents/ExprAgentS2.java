...
/**
 * 
 * Previously experimental agent emulating the playing of string 2 (the A string) on a Squirer guitar using Multi Xtal that has now effectively become the standard. 
 * 
 ...
 *
 */
public class ExprAgentS2 extends IntelligentAgent implements Externalizable {
	...
	public WaveForm genWaveQB()
	{
		...
		PhaseDistortionPacket pdcxf2 = new PhaseDistortionPacket(
				new SineWaveform(), 1.0,  0.042496240601503754 );
		PhaseDistortionPacket[] pdccxf2 = { pdcxf2 };
		WaveForm invf2 = new ArithGntWaveform( new Inverter(pdccxf2) , 4 );
		coefficients.add( invf2 );
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 1.06 * ( 2.5 / 2.0 ) * ( 2.5 / 1.7E-6 ) * ( 8578.000670298985 / -0.004774409616215518 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		
		
		
		WaveForm wave3 = MultiXtalS2.builtMultiXtalS2B2();
	...
	public WaveForm genWaveQA()
	{
		...
		WaveForm invf2 = new ArithGntWaveform( new Inverter(pdccxf2) , 4 );
		coefficients.add( invf2 );
		coefficientCoefficients.add( new ConstantNonClampedCoefficient( 1.06 * ( 2.5 / 2.0 ) * ( 2.5 / 0.001265 ) * ( -128160.00000975345 / 0.002872222763123461 ) / maxDivisor ));
		parameterCoefficients.add( new ConstantNonClampedCoefficient( 1.0 ));
		
		
		
		WaveForm wave3 = MultiXtalS2.builtMultiXtalS2A2();
		wave3 = new AdditiveWaveForm( wave3 , new ConstantNonClampedCoefficient( 1.0 / maxDivisor ) ,
				coefficients , coefficientCoefficients , parameterCoefficients );
