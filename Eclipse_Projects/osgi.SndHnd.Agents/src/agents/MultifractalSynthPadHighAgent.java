...
import noise.MultifractalWaveForm;
...
import waves.MultiXtalBuilder;
import waves.SawtoothWaveform;
import waves.TriangleWaveform;
...
/**
 * 
 * Agent using a multifractal to produce an effect similar to (one kind of) a synthesizer pad.
 * 
 * Note: the frequency of a note for the agent is a function of the underlying lattice spacing rather than the perceived audible pitch.
 * 
 * 
 * @author tgreen
 *
 */
public class MultifractalSynthPadHighAgent extends IntelligentAgent implements Externalizable {
	...
	public static void createInitialEnvelope( ArrayList<InterpolationPoint> interps )
	{
		...
		interps.add( new InterpolationPoint( 0.8 , 0.4 ) );
...
//		note.setRoughDraftWaveform(new SineWaveform());
	 ...
	 */
	public MultifractalSynthPadHighAgent() {
		super();
		
		GGroupWave noiseGrpA = new GGroupWave();
		{
			noiseGrpA.setName( "NoiseGroup" );
			...
			gm.load( vg );
			noiseGrpA.getEdit().getWaveOut().performAssign( gm );
			AazonTransChld.initialCoords(AczonUnivAllocator.allocateUniv(),noiseGrpA.getEdit().getElem(), null, noiseGrpA.getEdit().getWaveOut());
		}
		
		GGroupWave noiseGrpB = new GGroupWave();
		{
			noiseGrpB.setName( "NoiseGroup" );
			GHashWhiteWaveForm white = new GHashWhiteWaveForm();
			white.load(1E+5,0.53147638582379454239574 / 0.5, 0.637724401110901,65599);
			GWardNoiseWaveform vg = new GWardNoiseWaveform();
			vg.load( white , 0.5 );
			GMarbleWave gm = new GMarbleWave();
			gm.load( vg );
			noiseGrpB.getEdit().getWaveOut().performAssign( gm );
			AazonTransChld.initialCoords(AczonUnivAllocator.allocateUniv(),noiseGrpB.getEdit().getElem(), null, noiseGrpA.getEdit().getWaveOut());
		}
		
		final FrequencyModulationWaveForm fpdcxa1 = new FrequencyModulationWaveForm( noiseGrpA.genWave( new HashMap() ) , new ConstantNonClampedCoefficient( 4.0 )  );
		final FrequencyModulationWaveForm fpdcxa2 = new FrequencyModulationWaveForm( noiseGrpA.genWave( new HashMap() ) , new ConstantNonClampedCoefficient( 4.0 * 1.05 )  );
		
		final FrequencyModulationWaveForm fpdcxb1 = new FrequencyModulationWaveForm( noiseGrpB.genWave( new HashMap() ) , new ConstantNonClampedCoefficient( 4.0 )  );
		final FrequencyModulationWaveForm fpdcxb2 = new FrequencyModulationWaveForm( noiseGrpB.genWave( new HashMap() ) , new ConstantNonClampedCoefficient( 4.0 * 1.05 )  );
		
		final ArrayList<NonClampedCoefficient> coefficientsa = new ArrayList<NonClampedCoefficient>();
		final ArrayList<NonClampedCoefficient> coefficientCoefficientsa = new ArrayList<NonClampedCoefficient>();
		final ArrayList<NonClampedCoefficient> parameterCoefficientsa = new ArrayList<NonClampedCoefficient>();
		
		final ArrayList<NonClampedCoefficient> coefficientsb = new ArrayList<NonClampedCoefficient>();
		final ArrayList<NonClampedCoefficient> coefficientCoefficientsb = new ArrayList<NonClampedCoefficient>();
		final ArrayList<NonClampedCoefficient> parameterCoefficientsb = new ArrayList<NonClampedCoefficient>();
		
		
		coefficientsa.add( fpdcxa2 );
		coefficientCoefficientsa.add( new ConstantNonClampedCoefficient( 0.5 ) );
		parameterCoefficientsa.add( new ConstantNonClampedCoefficient( 1.0 ) );
		
		
		coefficientsb.add( fpdcxb2 );
		coefficientCoefficientsb.add( new ConstantNonClampedCoefficient( 0.5 ) );
		parameterCoefficientsb.add( new ConstantNonClampedCoefficient( 1.0 ) );
		
		
		final WaveForm inva = new AdditiveWaveForm( fpdcxa1 , new ConstantNonClampedCoefficient( 0.5 ) , coefficientsa , coefficientCoefficientsa ,  parameterCoefficientsa  );
		final WaveForm invb = new AdditiveWaveForm( fpdcxb1 , new ConstantNonClampedCoefficient( 0.5 ) , coefficientsb , coefficientCoefficientsb ,  parameterCoefficientsb  );
	...
	{
		NoteInitializer fa = new NoteInitializer( NoteTable.getCloseNoteDefaultScale_Key(6,
				NoteTable.STEPS_C) );
